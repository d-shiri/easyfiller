...
language** by editing the config (see [Use another language](#use-another-language)).

From a word in the source field (**Back** by default):

- **Generate** (`Ctrl+Shift+G`) — call the **Claude** CLI to fill the meaning
...
1. **Claude Code CLI** installed and signed in (`claude` works in a terminal). Auth uses your
   Claude Code login — no API key is stored. The add-on prefers the self-contained native
   binary `~/.local/bin/claude` and repairs `PATH` for node-based shims automatically.
...
voice/speed/pitch, `normalize_word`, or `claude_path`. Full list in
[`config.md`](config.md).
...
- **`claude_prompt`** — set a custom prompt for the new language. It must keep the
...
- `claude_client.py` — runs the `claude` CLI and parses JSON (no Anki imports; runs in a
  background thread).
